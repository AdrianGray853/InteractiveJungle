...
using UnityEngine.UI;
    ...
    public class OnBoardingController : MonoBehaviour
    {
    	...
    	public Transform SelectLetterPanel;
    	...
    	public void AcceptAge()
    	{
    		...
    		if (InGame)
    		{
    			...
    			TransitionManager.Instance.ShowFade(Color.white, 1.0f, () =>
    			{
    				if (!SystemInfo.deviceModel.Contains("iPad"))
      ...
      //      }
    	...
    	{
    		//BirthdayVerticalPanel.DOScale(Vector3.zero, 0.5f).SetEase(Ease.OutBack).OnComplete(() => BirthdayVerticalPanel.gameObject.SetActive(false));
    		AdPanel.gameObject.SetActive(false);
    		SelectLetterPanel.gameObject.SetActive(true);
    	}

    	public void ShowAgeSelection()
    	{
    		SelectLetterPanel.gameObject.SetActive(false);
    		SelectAgePanel.gameObject.SetActive(true);
    	}
    	public void SelectAgeRange(int range)
    	{
    		ProgressManager.Instance.SetSelectedAgeRange(range);
