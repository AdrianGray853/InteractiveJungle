...
using UnityEngine;
    ...
    public class ScrollPanelShape : MonoBehaviour
    {
        public float Size;
        ...
        public OutlineControllerShape Panel;
    	...
    	void UpdateDragInfo()
    	{
            ...
            if (DragInfo.IsDragging || DragInfo.IsScrolling)
    		{
                for (int i = 0; i < Input.touchCount; i++)
    			{
    				...
    				{
                        Vector3 touchPos = DragManagerShape.GetWorldSpacePos(touch.position).SetZ(0);
    	...
    	void UpdateDrag()
    	{
            if (DragInfo.Pressed)
    		{
                ...
                for (int i = 0; i < Items.Count; i++)
    			{
                    ...
                    if (UtilsShape.IsPointInBox2D(DragInfo.StartPos, Items[i].collider.transform.position, boxSize)) 
    				{
            ...
            else if (DragInfo.Released)
    		{
                if (DragInfo.Item != null)
    			{
                    if (!GameManagerShape.Instance.GetCurrentLeveDropZonel().CanDrop(DragInfo.Item.collider))
    				{
