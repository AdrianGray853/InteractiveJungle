...
using UnityEngine;
    ...
    public class DragHelperTouch
    {
        ...
        public DragHelperTouch(int fingerId, Transform transform, Vector3 attachPosition, Vector3 offset, bool isRigidBody = false)
        {
        ...
        public void Update(Vector3 newPosition)
        {
            if (DraggingTransform != null)
            {
                if (IsRigidBody)
    			{
    				...
    				Vector2 tgtVel = Vector3.ClampMagnitude(DragManagerTouch.Instance._toVel * dist, DragManagerTouch.Instance._maxVel);
    				Vector2 error = tgtVel - rigidBody.GetPointVelocity(forcePos);
    				Vector2 force = Vector3.ClampMagnitude(DragManagerTouch.Instance._gain * error, DragManagerTouch.Instance._maxForce);
    				rigidBody.AddForceAtPosition(force, forcePos);
    			}
                else
    			{
                    newPosition = DragManagerTouch.ClampPositionToSafeScreen(newPosition + DraggingTransform.rotation * offset + RestrictOffset) - RestrictOffset;
