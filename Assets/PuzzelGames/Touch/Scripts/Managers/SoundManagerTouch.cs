...
using UnityEngine;
    ...
    public class SoundManagerTouch : MonoBehaviour
    {
    	private static SoundManagerTouch _instance = null;
    	public static SoundManagerTouch Instance
    	{
    		get
    		{
    			if (_instance == null)
    	...
    	private void Awake()
        {
    		...
    		Debug.LogWarning("Music is disabled in Editor, look in SoundManagerTouch.cs:Start()!");
    #endif
    	}
    	...
    	void Update()
    	{
    		if (queueTimer > 0)
    		{
    			...
    			{
    				AudioQueue peek = queue.Peek();
    				if (peek.Delay > 0)
    				{
    					queueTimer = peek.Delay;
    					peek.Delay = 0; // reset
    				}
    				else
    				{
    	...
    	public class SFXStack
        {
            ...
            {
    			AudioClip clip = SoundManagerTouch.Instance.GetClip(name);
    			if (clip != null)
    				PreviousDuration = clip.length;
    			StackSequence.AppendCallback(() => SoundManagerTouch.Instance.PlaySFX(name, volume, Category, Priority, this));
    	...
    	public void AddSFXToQueue(string name, float volume = 1.0f, string category = "sfx", int priority = 0, float delay = 0f)
    	{
    		if (queueTimer <= 0)
    		{ // Play instantly
    		...
    		else
    		{
    			...
    			*/
    			...
    			if (audioDesc != null)
    			{
    				...
    				queue.Enqueue(new AudioQueue() { Description = audioDesc, Volume = volume, IsSpatial = false, Category = category, Priority = priority, Delay = delay });
    			}
    			else
    			{
    	...
    	public void AddSFXToQueue(string name, Vector3 position, float volume = 1.0f, float delay = 0f)
    	{
    		if (queueTimer <= 0)
    		{ // Play instantly
    			AudioClip clip = GetClip(name);
    		...
    		else
    		{
    			...
    			if (audioDesc != null)
    			{
    				queue.Enqueue(new AudioQueue() { Description = audioDesc, Volume = volume, IsSpatial = true, Position = position, Delay = delay });
    			}
    		}
    	}

    #region DrawingStuff
    	public static string GetCheeringVoice()
    	{
    		string[] sounds =
    		{
    			"youre_talented", "you_got_eye_for_it", "briliant", "lovely", "great", "excelent",
    			"lets_continue", "incredible_work", "true_artist", "masterpiece_complete",
    			"artwork_amazing", "look_accomplish_amazing"
    		};
