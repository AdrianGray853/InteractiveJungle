...
using UnityEngine.UI;
    ...
    public static class UtilsTouch
    {
    	...
    	public static Rect GetWorldRectGameSpace(this RectTransform rectTransform)
        {
    		Vector3[] corners = new Vector3[4];
    		rectTransform.GetWorldCorners(corners);
    	...
    	public static bool In<T>(T element, params T[] set)
    	{
    		return set.Contains(element);
    	}

    	public static bool In<T>(IEnumerable<T> element, params T[] set)
    	{
    		return element.Intersect(set).Count() > 0;
    	}
    	...
    	public static bool IsAmazonFire()
    	{
    		return SystemInfo.deviceModel.Contains("Amazon");
    	}

    	public static bool IsTablet()
    	{
    		Debug.Log("Is Tablet check! " + (IsIPad() || IsAmazonFire()));
    #if UNITY_EDITOR
    		return false;
    #else
    		return IsIPad() || IsAmazonFire();
    #endif
    	}
    }
