...
public class VehicleDragging : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    public VehicleItem item;
    ...
    public void OnEndDrag(PointerEventData eventData)
    {
        ...
        if (IsWithinBounds(finalPos))
        {
            ...
            spawnedObj.GetComponent<VehicleItemUi>().Item = item;
            int index = GameManager.instance.house.activeItems.Count;
            string uniqueKey = $"{item.itemName}:{index}";
            item.postionkey = uniqueKey;
            GameManager.instance.vehicle.AddActiveItem(item);
            GameManager.instance.vehicle.SaveData(item);
